...
class Evaluacion(Frame):
	def __init__(self, master,model):
		...
		m_title=Label(Manage_Frame,text="Formulario de evaluación", bg="gray17", fg="black", font=("Lucida Sans",20,"bold"))
		...
		self.identificaciones_table=ttk.Treeview(Table_Frame,columns=("ID","Responsable", "Fecha", "Clase", "Comentarios","Identificaciones","Salida"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
		...
		self.identificaciones_table.heading("Identificaciones", text="Identificaciones")
		self.identificaciones_table.heading("Salida", text="Salida")
		...
		self.identificaciones_table.column("Identificaciones", width=100)
		self.identificaciones_table.column("Salida", width=100)
	...
	def regresar(self):
		...
		if self.tipo_user == 1:
			self.controlador.vistaEvaluacion_vistaMenuA()
		if self.tipo_user == 2:
			self.controlador.vistaEvaluacion_vistaMenuE()
	...
	def abrir_archivo(self):
		...
		if (responsable == "" or responsable.isspace() or responsable != self.controlador.rescatarNombreUsuario()) or (fecha == "") or (clase == "" or clase.isspace()) or (comentarios == "" or comentarios.isspace()):
			tkinter.messagebox.showerror("Alerta", "Algunos valores no coinciden")

		else:
			archivo_avierto = filedialog.askopenfilename(initialdir="/",title="Seleccione un video", filetypes=(("mp4 files", "*.mp4"),("all files", "*.*")))
			chekpoint = filedialog.askopenfilename(initialdir="..\Deteccion\checkpoints", title="Seleccione un chekpoint", filetypes=(("pth files", "*.pth"), ("all files", "*.*")))

			self.responsable.set("")
			self.clase.set("")
			self.txt_comentarios.delete("0.0", "end")
			self.controlador.comenzar_identificacion(archivo_avierto,responsable,fecha,clase,comentarios,chekpoint)

	def mostrar(self):
		nombreUser = self.controlador.rescatarNombreUsuario()
		datos = self.controlador.consultaEspecificaEval(nombreUser)
		self.identificaciones_table.delete(*self.identificaciones_table.get_children())
		i = -1
		for dato in datos:
			i = i + 1
